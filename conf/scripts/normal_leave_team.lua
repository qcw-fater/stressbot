-- normal_leave_team.lua: 普通模式每轮入口、正常结算和异常恢复共用的退队动作。
-- teamId 是可能超过 2^53 的 int64 雪花 ID，必须保持字符串/原始值直传，禁止 tonumber。
...
function execute(r)
    ...
    if err == nil then
        clearTeamState()
        log.info("普通模式退队成功: teamId=" .. tostring(teamId))
        return nil
    end
    ...
    if code == 311 or code == 288 then
        ...
        if cancelErr ~= nil then
            log.warn("普通模式取消匹配失败，继续重试退队: teamId=" .. tostring(teamId)
        ...
        if err2 == nil then
            clearTeamState()
            log.info("普通模式取消匹配后退队成功: teamId=" .. tostring(teamId))
            return nil
        end
        if tonumber(err2.code) == 308 then
            clearTeamState()
            log.warn("普通模式退队时服务端已无该队伍，清理本地状态: teamId=" .. tostring(teamId))
            return nil
        end
        log.warn("普通模式取消匹配后退队仍失败，保留 teamId: teamId=" .. tostring(teamId)
    ...
    log.warn("普通模式退队失败，保留 teamId: teamId=" .. tostring(teamId)
