...
function execute(r)
    ...
    if code < 0 then
        log.error("SelectRole HTTP 请求失败: code=" .. tostring(code) .. " body=" .. body_preview(body))
        return 3, sent, recv  -- 3=SEND_FAILED
    end

    if code < 200 or code >= 300 then
        log.error("SelectRole HTTP 状态异常: code=" .. tostring(code) .. " body=" .. body_preview(body))
        return 54, sent, recv
    end

    local ok, resp = pcall(json.decode, body)
    if not ok or not resp then
        log.error("SelectRole JSON 解析失败: " .. body_preview(body))
        return 54, sent, recv
    end
