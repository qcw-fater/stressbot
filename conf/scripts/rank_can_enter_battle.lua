-- rank_can_enter_battle.lua: 排位进战斗门禁（boolean lua 条件），兼诊断
-- 记录战斗四件套的实际值/类型，返回是否齐备。ListenStartLoading 成功（已下发地址/ID/密钥/会话）后应全部就绪。
...
function execute(r)
    ...
    local detail = "排位进战门禁: battleId=" .. tostring(battleId) .. "(" .. type(battleId) .. ")"
