...
//
// 输出 JSON 数组：进程内每个 FrozenCache（正常一个 Factory 一个）一条快照。
// 判读方法：
...
var (
	statsMu     sync.Mutex
	statsCaches []*FrozenCache
)

func registerCacheForStats(c *FrozenCache) {
	statsMu.Lock()
...
func init() {
	http.HandleFunc("/debug/dedup", func(w http.ResponseWriter, _ *http.Request) {
		statsMu.Lock()
		caches := make([]*FrozenCache, len(statsCaches))
