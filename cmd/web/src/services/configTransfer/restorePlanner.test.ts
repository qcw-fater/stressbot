...
describe('findDuplicate', () => {
  ...
  it('matches either an id or an exact name', () => {
    ...
    expect(findDuplicate(current, item('id-1', 'Renamed'), identity))
      .toMatchObject({ kind: 'one', matches: [current[0]] });
    expect(findDuplicate(current, item('id-3', 'B'), identity))
      .toMatchObject({ kind: 'one', matches: [current[1]] });
  });

  it('reports ambiguity when id and name hit different targets', () => {
    const current = [item('id-1', 'A'), item('id-2', 'B')];

    expect(findDuplicate(current, item('id-1', 'B'), identity))
      .toMatchObject({ kind: 'ambiguous', matches: current });
  });
});
...
describe('planCollectionMerge', () => {
  ...
  it('applies per-item overwrite, keep-copy, and skip choices immutably', () => {
    ...
    const copied = applyConflictChoices(pending, {
    ...
    expect(copied.finalItems).toEqual([
      ...current,
      item('copy-1', 'A（副本）', 'new-a'),
    ]);
    expect(copied.stats).toMatchObject({ copied: 1, skipped: 1 });

    const overwritten = applyConflictChoices(pending, {
  ...
  it('keeps an already unique source name when only the id conflicts', () => {
    ...
    const resolved = applyConflictChoices(pending, {
...
describe('singleton planning', () => {
  it('does not allow keeping a second singleton copy', () => {
    ...
    expect(() => applyConflictChoices(pending, {
      [conflict.id]: 'keep-copy',
    })).toThrow('单例分区不支持保留副本');
  });

  it('clears an existing singleton only during complete restore', () => {
    const replaced = planSingleton(
      { value: 'old' },
      null,
      'replace',
      'overwrite',
...
describe('nextCopyName', () => {
  it('allocates a unique name before the extension', () => {
    expect(nextCopyName(
      'login.lua',
      new Set(['login.lua', 'login（副本）.lua']),
    )).toBe('login（副本 2）.lua');
  });
});
