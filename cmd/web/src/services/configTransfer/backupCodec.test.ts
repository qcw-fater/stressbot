...
describe('parseBackupText', () => {
  it('parses a valid partial backup and invokes nested validation', () => {
    ...
    expect(validateSection).toHaveBeenCalledWith('protoFiles', [{ name: 'login.proto' }]);
  });
  ...
  it('rejects invalid JSON and a wrong backup kind', () => {
    ...
    expect(() => parseBackupText(JSON.stringify(backup({ kind: 'flow' })))).toThrow(
      '不是 stressbot 配置备份文件',
    );
  });

  it('rejects a newer schema version', () => {
    expect(() => parseBackupText(JSON.stringify(backup({ schemaVersion: 2 })))).toThrow(
      '备份格式版本 2 高于当前支持版本 1',
    );
  });

  it('rejects an invalid export time', () => {
    expect(() => parseBackupText(JSON.stringify(backup({ exportedAt: 'yesterday' })))).toThrow(
      '导出时间无效',
    );
  });
...
describe('createBackupBundle', () => {
  ...
  it('names the failing selected section and does not return a partial bundle', async () => {
    ...
    await expect(createBackupBundle(['protoFiles', 'luaFiles'], registry)).rejects.toThrow(
      'Lua 脚本读取失败',
    );
  });
});
