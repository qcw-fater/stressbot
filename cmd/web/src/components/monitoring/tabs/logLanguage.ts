...
export function registerLogLanguage(monaco: Monaco) {
  ...
  monaco.languages.setMonarchTokensProvider(LANGUAGE_ID, {
    tokenizer: {
      ...
      sep1: [
        { include: '@tsGuard' },
        [/  /, { token: '', next: '@expectLevel' }],
      ],
      expectLevel: [
        { include: '@tsGuard' },
        [/dpanic /, { token: 'log-level-fatal', next: '@fieldCaller' }],
        [/panic  /, { token: 'log-level-fatal', next: '@fieldCaller' }],
        [/fatal  /, { token: 'log-level-fatal', next: '@fieldCaller' }],
        [/error  /, { token: 'log-level-error', next: '@fieldCaller' }],
        [/warn   /, { token: 'log-level-warn', next: '@fieldCaller' }],
        [/info   /, { token: 'log-level-info', next: '@fieldCaller' }],
        [/debug  /, { token: 'log-level-debug', next: '@fieldCaller' }],
      ],
      fieldCaller: [
        { include: '@tsGuard' },
        [/\S+/, { token: 'log-source', next: '@sep3' }],
        [/  /, { token: '', next: '@fieldService' }],
      ],
      sep3: [
        { include: '@tsGuard' },
        [/  /, { token: '', next: '@fieldService' }],
