...
const networkModule: LuaModule = {
  ...
  functions: [
    ...
    },
    {
      ...
      returns: 'data : string|userdata|nil',
      summary: '等待 UDP 服务端推送（轮询 ListenRefs 预缓存）',
      detail: 'nil 表示超时或 context 取消。需先调用 ensure_udp_listener() 注册监听占位。',
    },
    {
      name: 'http_request',
      module: 'network',
    ...
    },
    {
      ...
      detail: 'nil 表示超时或 context 取消。需先调用 ensure_tcp_listener() 注册监听占位。阻塞期间会释放 luaMu 让心跳/回调继续。',
    },
    {
