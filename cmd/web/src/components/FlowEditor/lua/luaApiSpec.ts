...
const networkModule: LuaModule = {
  ...
  functions: [
    ...
    },
    {
      ...
      module: 'network',
      params: [
        { name: 'service', type: 'string', doc: 'TCP 连接名' },
        { name: 'route', type: 'table', optional: true, doc: '可选路由 {cmd, act}' },
      ],
      returns: 'boolean',
      summary: '发送空包交换 TCP 密钥',
    },
    {
      name: 'request',
      module: 'network',
      ...
      returns: 'code, data : (number, string|userdata)',
      summary: 'TCP 请求-响应',
      detail: 'code: 0=成功 / -1=请求失败 / -2=解析失败（data 为原始字节）',
      example: `local code, resp = network.request("logic", {cmd=1, act=1}, msg, "login.LoginS2C")`,
    },
    {
      name: 'tcp_send',
      module: 'network',
    ...
    },
    {
      ...
      params: [
        { name: 'service', type: 'string', doc: '连接名' },
        { name: 'data', type: 'string', doc: '原始字节串（不做编码）' },
      ],
      returns: 'code (number)',
      summary: 'UDP 发送原始字节',
    },
    {
      ...
      returns: 'code (number)',
      summary: 'UDP 发送（带路由编码）',
    },
    {
      name: 'http_post',
      module: 'network',
      params: [
        { name: 'path', type: 'string', doc: '请求路径（如 "/api/login"）' },
        { name: 'form_data', type: 'table', doc: '表单数据 table' },
      ],
      returns: 'status_code, body : (number, string)',
      summary: 'HTTP POST 表单',
    },
    {
      name: 'wait_listen',
      module: 'network',
    ...
    },
    {
      ...
      returns: '-',
      summary: '为 responseKey 注册监听占位',
      detail: '通常无需手动调用，由 declarative 监听节点自动处理',
    },
    {
