...
export function HistoryDetailView({ id, onChange }: HistoryDetailViewProps) {
  ...
  const actionTable = useMemo(() => {
    ...
    const columns: ColumnsType<ActionMetric> = [
      ...
      { title: 'RTT avg(ms)', key: 'avgMs', width: 92, sorter: (a, b) => a.rtt.avgMs - b.rtt.avgMs, render: (_, r) => <span style={NUMERIC_STYLE}>{r.rttSampleCount > 0 ? fmtMs(r.rtt.avgMs) : '—'}</span> },
      ...
      { title: 'encode(ms)', dataIndex: 'encodeAvgMs', key: 'encodeAvgMs', width: 92, sorter: (a, b) => a.encodeAvgMs - b.encodeAvgMs, render: (v: number) => <span style={NUMERIC_STYLE}>{fmtMs(v)}</span> },
      { title: 'decode(ms)', dataIndex: 'decodeAvgMs', key: 'decodeAvgMs', width: 92, sorter: (a, b) => a.decodeAvgMs - b.decodeAvgMs, render: (v: number) => <span style={NUMERIC_STYLE}>{fmtMs(v)}</span> },
      { title: 'parse/store(ms)', dataIndex: 'parseStoreAvgMs', key: 'parseStoreAvgMs', width: 120, sorter: (a, b) => a.parseStoreAvgMs - b.parseStoreAvgMs, render: (v: number) => <span style={NUMERIC_STYLE}>{fmtMs(v)}</span> },
      {
