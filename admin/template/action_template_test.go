...
var validActionTemplateSave = ActionTemplateSaveRequest{
	Name:    "登录请求",
	Pattern: flowdef.PatternTCPRequest,
...
func TestValidateActionTemplate(t *testing.T) {
	tests := []struct {
		...
		{name: "mismatched pattern", mutate: func(r *ActionTemplateSaveRequest) { r.Pattern = flowdef.PatternUDPSend }},
	}
	...
	invalidBodies := []struct {
		...
		{name: "missing service", pattern: flowdef.PatternTCPRequest, data: `{"pattern":"tcpRequest","route":{},"s2cProto":"LoginS2C"}`},
		{name: "missing route", pattern: flowdef.PatternTCPRequest, data: `{"pattern":"tcpRequest","service":"logic","s2cProto":"LoginS2C"}`},
		{name: "missing address", pattern: flowdef.PatternTCPConnect, data: `{"pattern":"tcpConnect","service":"logic"}`},
		{name: "missing c2s proto", pattern: flowdef.PatternTCPSend, data: `{"pattern":"tcpSend","service":"logic","route":{}}`},
		{name: "missing s2c proto", pattern: flowdef.PatternTCPRequest, data: `{"pattern":"tcpRequest","service":"logic","route":{}}`},
		{name: "blank lua script", pattern: flowdef.PatternLua, data: `{"pattern":"lua","script":"  "}`},
		{name: "missing clear state keys", pattern: flowdef.PatternClearState, data: `{"pattern":"clearState","keys":[]}`},
		{name: "missing http url", pattern: flowdef.PatternHTTPRequest, data: `{"pattern":"httpRequest"}`},
		{name: "invalid http method", pattern: flowdef.PatternHTTPRequest, data: `{"pattern":"httpRequest","url":"https://example.com","method":"PUT"}`},
		{name: "invalid http content type", pattern: flowdef.PatternHTTPRequest, data: `{"pattern":"httpRequest","url":"https://example.com","contentType":"xml"}`},
	}
...
func TestActionTemplateStoreCreateAndList(t *testing.T) {
	...
	store := NewActionTemplateStore(db, testNextID)

	mock.ExpectExec(`(?s)INSERT INTO action_template`).
		WithArgs(sqlmock.AnyArg(), "登录请求", nil, flowdef.PatternTCPRequest, []byte(validActionTemplateSave.Data), sqlmock.AnyArg(), sqlmock.AnyArg()).
	...
	mock.ExpectQuery(`(?s)SELECT id, name, description, pattern, data_json, created_at, updated_at.*FROM action_template`).
		WillReturnRows(sqlmock.NewRows([]string{"id", "name", "description", "pattern", "data_json", "created_at", "updated_at"}).
			AddRow("a", "A", nil, flowdef.PatternTCPRequest, []byte(validActionTemplateSave.Data), now, now))
...
func TestActionTemplateStoreUpdateNotFound(t *testing.T) {
	...
	_, err = NewActionTemplateStore(db, testNextID).Update(context.Background(), "missing", validActionTemplateSave)
