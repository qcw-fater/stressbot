...
func TestManagementOpenAPIFreezesCurrentRoutes(t *testing.T) {
	...
	if got := len(operationIDs); got != 62 {
		t.Fatalf("operation count = %d, want 62", got)
	}
...
func TestManagementOpenAPIOmitsObsoleteBaselineConfigEndpoint(t *testing.T) {
	spec := loadManagementOpenAPI(t)
	if spec.Paths.Find("/sbot/baseline/config.json") != nil {
		t.Fatal("obsolete baseline config endpoint remains in management OpenAPI")
	}
}
...
func loadManagementOpenAPI(t *testing.T) *openapi3.T {
	t.Helper()
	spec, err := openapi3.NewLoader().LoadFromData(adminapi.AdminSpec())
	if err != nil {
		t.Fatal(err)
	}
