...
func TestAggregatorSeparatesCumulativeFactsFromFreshLiveRates(t *testing.T) {
	...
	registry := &AgentRegistry{agents: make(map[string]*AgentNode)}
	for _, id := range []string{"a", "b"} {
		registry.agents[id] = &AgentNode{ID: id, Status: AgentBusy, CurrentTaskID: "task-1"}
	}
	...
	aggregator := NewMetricsAggregator(registry, store, clock.Now)
...
func TestAggregateSystemUsesAssignedScopeAndFreshAdminReceiptTimes(t *testing.T) {
	...
	registry := &AgentRegistry{agents: map[string]*AgentNode{
		"a":       systemAgentNode("a", "node-a", AgentBusy, 4, "5s", now.Add(-10*time.Second), systemSnapshotValues(20, 100, 50, 10, 200, 5, 1, 2)),
		"b":       systemAgentNode("b", "node-b", AgentBusy, 8, "5s", now.Add(-16*time.Second), systemSnapshotValues(90, 100, 90, 80, 900, 90, 9, 9)),
		"outside": systemAgentNode("outside", "outside", AgentBusy, 64, "5s", now, systemSnapshotValues(99, 1_000, 999, 99, 9_999, 999, 99, 99)),
	}}
	aggregator := NewMetricsAggregator(registry, nil, func() time.Time { return now })
...
func TestAggregateSystemAggregatesEachValidFieldWithExplicitCoverage(t *testing.T) {
	...
	registry := &AgentRegistry{agents: map[string]*AgentNode{
		"a": systemAgentNode("a", "node-a", AgentBusy, 4, "10s", now.Add(-5*time.Second), a),
		"b": systemAgentNode("b", "node-b", AgentBusy, 12, "10s", now.Add(-5*time.Second), b),
	}}
	aggregator := NewMetricsAggregator(registry, nil, func() time.Time { return now })
...
func TestAggregateSystemReturnsAnEmptyAgentArrayForAnEmptyScope(t *testing.T) {
	registry := &AgentRegistry{agents: map[string]*AgentNode{}}
	aggregator := NewMetricsAggregator(registry, nil, time.Now)
...
func systemAgentNode(id, name string, status AgentStatus, cpus int, interval string, receivedAt time.Time, snapshot SystemSnapshot) *AgentNode {
	return &AgentNode{
		ID:              id,
		Name:            name,
		Status:          status,
		SystemInterval:  interval,
		StaticInfo:      StaticInfo{NumCPU: cpus},
