...
func TestAcceptTaskReportDoesNotClearNewTask(t *testing.T) {
	...
	if err := agents.Register(&agent.AgentNode{
		ID:            "agent-1",
		Status:        agent.AgentBusy,
	...
	if err := service.Accept(TaskCompletionReport{AgentID: "agent-1", TaskID: "old-task", Result: ResultCompleted}); err != nil {
	...
	if node.Status != agent.AgentBusy {
		t.Errorf("Status = %q, want %q", node.Status, agent.AgentBusy)
	}
}
