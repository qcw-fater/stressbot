...
func TestActionSnapshotPreserveKeepsIDAndTimes(t *testing.T) {
	...
	prepared, err := prepareActionSnapshotItems(nil, incoming, TemplateIDPreserve, now.Add(time.Hour), func() string { return "generated" })
	if err != nil {
		t.Fatal(err)
	}
...
func TestActionSnapshotGenerateMissingCreatesServerID(t *testing.T) {
	...
	prepared, err := prepareActionSnapshotItems(nil, []ActionTemplate{incoming}, TemplateIDGenerateMissing, now, func() string { return "server-id" })
	if err != nil {
		t.Fatal(err)
	}
...
func TestActionSnapshotGenerateMissingPreservesUnchangedTimes(t *testing.T) {
	...
	prepared, err := prepareActionSnapshotItems([]ActionTemplate{current}, []ActionTemplate{incoming}, TemplateIDGenerateMissing, updated.Add(time.Hour), generateID)
	if err != nil {
		t.Fatal(err)
	}
...
func TestListenSnapshotGenerateMissingUpdatesChangedExistingItem(t *testing.T) {
	...
	incoming.CreatedAt, incoming.UpdatedAt = time.Time{}, time.Time{}
	prepared, err := prepareListenSnapshotItems([]ListenTemplate{current}, []ListenTemplate{incoming}, TemplateIDGenerateMissing, now, generateID)
	if err != nil {
		t.Fatal(err)
	}
...
func TestTemplateSnapshotRejectsDuplicateNamesBeforeBegin(t *testing.T) {
	...
	if _, err := prepareActionSnapshotItems(nil, items, TemplateIDPreserve, now, generateID); err == nil {
...
func TestTemplateSnapshotRollsBackWhenInsertFails(t *testing.T) {
	...
	mock.ExpectRollback()
	_, err = NewActionTemplateStore(db).ReplaceSnapshot(context.Background(), ReplaceTemplateSnapshotRequest[ActionTemplate]{ExpectedRevision: revision, IDPolicy: TemplateIDPreserve, Items: []ActionTemplate{current}})
	if err == nil {
