...
func taskAssignmentToProto(src TaskAssignment) (*controlv1.TaskAssignment, error) {
	if src.TaskID == "" || src.StartIndex < 0 || src.TotalBots <= 0 {
		return nil, fmt.Errorf("任务分配参数无效")
	}
	out := &controlv1.TaskAssignment{
		TaskId:        src.TaskID,
...
func rampUpToProto(src *RampUpConfig) *controlv1.RampUpConfig {
	if src == nil {
		return nil
	}
	out := &controlv1.RampUpConfig{Stages: make([]*controlv1.RampUpStage, 0, len(src.Stages))}
	for _, stage := range src.Stages {
		out.Stages = append(out.Stages, &controlv1.RampUpStage{
...
func sharedAssignmentToProto(src *SharedRuntimeAssignment) *controlv1.SharedRuntimeAssignment {
	if src == nil {
		return nil
	}
	return &controlv1.SharedRuntimeAssignment{
		RunId: src.RunID,
		Redis: &controlv1.RedisConfig{
			Host:                  src.Redis.Host,
...
func staticInfoFromProto(src *controlv1.StaticInfo) StaticInfo {
	if src == nil {
		return StaticInfo{}
	}
	return StaticInfo{
...
func systemSnapshotFromProto(src *controlv1.SystemMetricSnapshot) *SystemSnapshot {
	if src == nil {
		return nil
	}
...
func cleanupStatusFromProto(src *controlv1.CleanupStatus) *robot.CleanupStatus {
	if src == nil {
		return nil
	}
...
func finalReportFromProto(src *controlv1.FinalReport) (TaskCompletionReport, error) {
	if src == nil || src.AgentId == "" || src.TaskId == "" || src.ReportId == "" {
		return TaskCompletionReport{}, fmt.Errorf("最终报告缺少身份字段")
	}
	result, err := taskResultFromProto(src.Result)
	if err != nil {
		return TaskCompletionReport{}, err
	}
...
func taskResultFromProto(src controlv1.TaskResultCode) (TaskResult, error) {
	switch src {
	case controlv1.TaskResultCode_TASK_RESULT_CODE_COMPLETED:
		return ResultCompleted, nil
	case controlv1.TaskResultCode_TASK_RESULT_CODE_STOPPED:
		return ResultStopped, nil
	case controlv1.TaskResultCode_TASK_RESULT_CODE_FAILED:
