...
func TestMemoryCommandStoreEvictsOldestTerminalCommand(t *testing.T) {
	store := NewMemoryCommandStore(2)
	first := memoryTestCommand("command-1", "agent-1")
	second := memoryTestCommand("command-2", "agent-1")
	if err := store.CreateBatch(context.Background(), []*controlv1.Command{first, second}); err != nil {
		t.Fatalf("CreateBatch(first, second) error = %v", err)
	}
	if err := store.Acknowledge(context.Background(), first.CommandId, controlv1.CommandAckStatus_COMMAND_ACK_STATUS_APPLIED, ""); err != nil {
		t.Fatalf("Acknowledge(first) error = %v", err)
	}

	third := memoryTestCommand("command-3", "agent-1")
	if err := store.CreateBatch(context.Background(), []*controlv1.Command{third}); err != nil {
...
func TestMemoryCommandStoreNeverEvictsPendingCommand(t *testing.T) {
	store := NewMemoryCommandStore(1)
	if err := store.CreateBatch(context.Background(), []*controlv1.Command{memoryTestCommand("command-1", "agent-1")}); err != nil {
		t.Fatalf("CreateBatch(first) error = %v", err)
	}
	if err := store.CreateBatch(context.Background(), []*controlv1.Command{memoryTestCommand("command-2", "agent-1")}); !errors.Is(err, ErrCommandStoreFull) {
...
func TestMemoryCommandStoreInvalidBatchDoesNotEvictTerminalCommand(t *testing.T) {
	store := NewMemoryCommandStore(1)
	first := memoryTestCommand("command-1", "agent-1")
	if err := store.CreateBatch(context.Background(), []*controlv1.Command{first}); err != nil {
		t.Fatalf("CreateBatch(first) error = %v", err)
	}
	if err := store.Acknowledge(context.Background(), first.CommandId, controlv1.CommandAckStatus_COMMAND_ACK_STATUS_APPLIED, ""); err != nil {
		t.Fatalf("Acknowledge(first) error = %v", err)
	...
	if err := store.CreateBatch(context.Background(), []*controlv1.Command{invalid}); err == nil {
...
func TestMemoryCommandStorePendingUsesPerAgentSequenceOrder(t *testing.T) {
	store := NewMemoryCommandStore(4)
	first := memoryTestCommand("command-1", "agent-1")
	other := memoryTestCommand("command-2", "agent-2")
	last := memoryTestCommand("command-3", "agent-1")
	if err := store.CreateBatch(context.Background(), []*controlv1.Command{first, other, last}); err != nil {
...
func memoryTestCommand(id, agentID string) *controlv1.Command {
	return &controlv1.Command{
		CommandId: id,
		AgentId:   agentID,
		Body: &controlv1.Command_Shutdown{
			Shutdown: &controlv1.Shutdown{Reason: "test"},
		},
	}
}
