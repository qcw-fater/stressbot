...
**启动流程：** `cmd/agent/main.go` 加载配置 → Lua 协议适配器 → .proto 文件 → 流程配置 → 启动 gnet 网络引擎 → 创建 Lua 运行时池 → 创建 Robot Manager → 批量启动机器人。
...
- **`network/`** — 基于 gnet 的 TCP/UDP 连接层。`Client` 管理多服务 TCP 连接和单个 UDP 连接。`Connection` 处理收发、请求-响应匹配、持久化监听、per-connection 心跳。`Dialer` 封装 gnet 事件循环。
...
- `conf/flow/flow.json` — 流程图（节点、动作、回调）— 主要配置产物
