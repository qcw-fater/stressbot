...
func NewBundleCache(root string) (*BundleCache, error) {
	root = filepath.Join(root, "stressbot-bundles")
	if err := os.MkdirAll(root, 0o755); err != nil {
		return nil, fmt.Errorf("创建资源包缓存目录失败: %w", err)
	}
	return &BundleCache{root: root}, nil
}

func (c *BundleCache) Ensure(ctx context.Context, client controlv1.AgentBundleServiceClient, agentID string, digest []byte, size int64) (string, error) {
...
func (c *BundleCache) download(ctx context.Context, client controlv1.AgentBundleServiceClient, agentID string, digest []byte, size int64, archive string) error {
	return c.downloadAttempt(ctx, client, agentID, digest, size, archive, true)
}

func (c *BundleCache) downloadAttempt(ctx context.Context, client controlv1.AgentBundleServiceClient, agentID string, digest []byte, size int64, archive string, retryCorrupt bool) error {
	part := archive + ".part"
	...
	stream, err := client.DownloadBundle(ctx, &controlv1.BundleRequest{AgentId: agentID, Sha256: digest, Offset: offset})
	if err != nil {
		_ = file.Close()
