...
func (o *ReportOutbox) Offer(report *controlv1.FinalReport) (<-chan error, error) {
	o.mu.Lock()
	defer o.mu.Unlock()
	...
	pending := &pendingReport{report: proto.Clone(report).(*controlv1.FinalReport), done: make(chan error, 1)}
...
func (o *ReportOutbox) Snapshot() []*controlv1.FinalReport {
	o.mu.Lock()
	defer o.mu.Unlock()
	out := make([]*controlv1.FinalReport, 0, len(o.order))
	for _, id := range o.order {
		if pending := o.items[id]; pending != nil {
			out = append(out, proto.Clone(pending.report).(*controlv1.FinalReport))
		}
	}
	return out
}

func (o *ReportOutbox) Acknowledge(ack *controlv1.ReportAck) {
	o.mu.Lock()
